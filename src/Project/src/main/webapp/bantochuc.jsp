...
<%
    ...
    List<Contributor> organizers = service.getOrganizersWithStats();
    int totalOrganizers = organizers.size();
    
    int totalHours = 0;
    for (Contributor c : organizers) {
...
<html lang="vi">
  <head>
    ...
    <title>Ban Tổ Chức - Ánh Sáng Yêu Thương</title>
    ...
    <link
      rel="stylesheet"
      href="${pageContext.request.contextPath}/css/bantochuc_style.css"
    />
  </head>
  <body>
    <!-- ===== HEADER ===== -->
    ...
    <main class="page-content">
      <div class="container">
        ...
        <nav class="breadcrumb">
          ...
          <span class="divider">/</span>
          <span class="active bold">Ban tổ chức</span>
        </nav>

        <!-- Heading Block with Title and Stats -->
        <div class="page-heading-block">
          <div class="heading-info">
            <h1 class="page-title">BAN TỔ CHỨC <span class="heart-outline">🧡</span></h1>
            <p class="page-subtitle">Những người dẫn dắt và định hướng cho hành trình yêu thương của dự án.</p>
          </div>
          <div class="page-stats-container">
            <!-- Hộp 1: Số lượng thành viên -->
            <div class="page-stat-item green-theme">
              <div class="stat-icon-wrapper"></div>
              <div class="stat-info-text">
                <span class="stat-val"><%= totalOrganizers %></span>
                <span class="stat-lbl">Thành viên ban tổ chức</span>
              </div>
            </div>
            <!-- Hộp 2: Số chương trình tham gia -->
            <div class="page-stat-item blue-theme">
              <div class="stat-icon-wrapper"></div>
              <div class="stat-info-text">
                <span class="stat-val">15+</span>
                <span class="stat-lbl">Dự án hoàn thành</span>
              </div>
            </div>
            <!-- Hộp 3: Giờ cống hiến -->
            <div class="page-stat-item orange-theme">
              <div class="stat-icon-wrapper"></div>
              <div class="stat-info-text">
                <span class="stat-val"><%= String.format("%,d", totalHours).replace(',', '.') %>+</span>
                <span class="stat-lbl">Giờ cống hiến</span>
              </div>
            </div>
          </div>
        </div>

        <!-- Grid hiển thị tất cả Ban tổ chức -->
        <div class="organizers-page-grid">
          ...
          <div class="organizer-page-card">
            ...
            <p class="organizer-role-label"><%= contributor.getDescription() != null && !contributor.getDescription().isEmpty() ? contributor.getDescription() : "Thành viên ban tổ chức" %></p>
            <div class="organizer-page-stats">
              <div class="stat-col">
                <span class="stat-number"><%= contributor.getActivityCount() %></span>
                <span class="stat-label">Hoạt động</span>
              </div>
              <div class="stat-col">
                <span class="stat-number"><%= contributor.getTotalHours() %></span>
                <span class="stat-label">Giờ cống hiến</span>
              </div>
            </div>
          </div>
          <% } %>
